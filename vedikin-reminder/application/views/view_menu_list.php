	...
	<div class="col-xs-12">
		<div class="box">
			...
			<div class="box-header filter_div">
				<!-- Fillter area -->
				<form class="filter_form" id="frmListDataFilter" action="<?php echo SITE_URL ?>search_menu" method="post">
					<div class="row">
						<div class="col-sm-2">
						...
						<div class="col-sm-2">
							...
							<a href="<?php echo SITE_URL.'menu-list';?>" class="btn btn-info">Reset</a>
						</div>
					</div>
				</form>
				<!-- Fillter area end-->
			...
			<div class="box-body">
				...
				<?php } ?>
					...
					<tr>
						<?php 
							if (in_array("4", $user_rights)) { ?>
							<td><input type="checkbox" name="row-check" class="delete_checkbox" value="<?php echo $menu->menu_id;?>"></td>
						<?php } ?>
						<td><?php echo $i++; ?></td>
						<td><?php echo $menu->menu_name; ?></td>
						<td><?php echo $menu->menu_order;?></td>
						<td><?php echo getIsactiveButtonForList($menu->is_active,$menu->menu_id,DB_PREFIX.'menu','menu_id'); ?></td>
						<td>
							<?php 
								if (in_array("3", $user_rights)) {
									echo '<div class="btn-group" id="action">'.getActionButtonForList($menu->menu_id,'menu',array("E"))."</div>"; 
								}
								if(in_array("4", $user_rights)) { 
									echo '<div class="btn-group" id="action">'.getActionButtonForList($menu->menu_id,'menu',array("D"))."</div>";
								}
								if(in_array("1", $user_rights)) { 
									echo '<div class="btn-group" id="action">'.getActionButtonForList($menu->menu_id,'menu',array("V"))."</div>";
								}
							?>
						</td>
