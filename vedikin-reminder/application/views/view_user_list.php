	...
	<div class="col-xs-12">
		<div class="box">
			...
			<div class="box-header filter_div">
				<!-- Fillter area -->
				<form class="filter_form" id="frmListDataFilter" action="<?php echo SITE_URL ?>search_user" method="post">
					<div class="row">
						<div class="col-sm-2">
						...
						<div class="col-sm-2">
							...
							<a href="<?php echo SITE_URL.'user-list';?>" class="btn btn-info">Reset</a>
						</div>
					</div>
				</form>
				<!-- Fillter area end-->
			...
			<div class="box-body">
				...
				<?php } ?>
                    ...
                    <tbody>
						...
						<td><?php echo getIsactiveButtonForList($users->is_active,$users->user_id,DB_PREFIX.'user','user_id'); ?></td>
						<td>
							<?php 
								if (in_array("3", $user_rights)) {
									echo '<div class="btn-group" id="action">'.getActionButtonForList($users->user_id,'user',array("E"))."</div>"; 
								}
								if(in_array("4", $user_rights)) { 
									echo '<div class="btn-group" id="action">'.getActionButtonForList($users->user_id,'user',array("D"))."</div>";
								}
								if(in_array("1", $user_rights)) { 
									echo '<div class="btn-group" id="action">'.getActionButtonForList($users->user_id,'user',array("V"))."</div>";
								}
							?>
						</td>
